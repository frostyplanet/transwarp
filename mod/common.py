...
class TransWarpBase (object):
    ...
    def _on_err(self, conn, client):
        self.logger.error("client %s: peer %s %s" %
                          (client.client_id, conn.peer, conn.error))
    ...
    def stream_to_fix(self, stream_conn, fix_conn, client):
        ...
        def __send_and_close(client, data):
            if stream_conn.error is None:
                self.logger.error(
                    "client %s: peer close" % (client.client_id))
            else:
                self.logger.error("client %s: error %s" %
                                  (client.client_id, str(stream_conn.error)))
            if not data:
        ...
        if buf:
            ...
            if is_eof or stream_conn is not None:
                if stream_conn.error is not None:
                    self.logger.error(
                        "client %s: %s" % (client.client_id, str(stream_conn.error)))
                self.close_client(client)
            else:
                self.engine.watch_conn(stream_conn)
        ...
        try:
            def __head_err(fix_conn, *args):
                if not isinstance(fix_conn.error, PeerCloseError):
                    self.logger.error(
                        "client %s %s" % (client.client_id, fix_conn.error))
                self.close_client(client)
                return
...
#                    self.engine.remove_conn (fix_conn)
                    ...
                    self.engine.write_unblock(
                        stream_conn, data, __write_ok, self._on_err, cb_args=(client, ))
                except Exception, e:
                    self.logger.exception(
                        "client %s: response error %s" % (client.client_id, str(e)))
                    self.close_client(client)
                return
            self.engine.read_unblock(
                fix_conn, self.head_len, self._on_recv_head, __head_err,
