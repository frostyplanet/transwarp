...
#        print "client %s remote readable" % (client.client_id)
    ...
    def _on_server_connected(self, sock, client):
        ...
        def __write_ok(r_conn, *args):
            self.engine.read_unblock(
                r_conn, self.head_len, __on_read_head, self._on_err, cb_args=(client, ))
            return
        self.engine.write_unblock(
            r_conn, buf, __write_ok, self._on_err, cb_args=(client,))
        return self._send_sock5_reply(client, 0)

    def _connect_server(self, host, port, cli_conn):
        # self.logger.debug ("connecting server %s for %s:%s" %
        ...
        def __on_connect_error(err, *args):
            self.logger.error("client %s cannot connect to server, %s" %
                              (client.client_id, str(err)))
            return self._send_sock5_reply(client, errno.EHOSTUNREACH)
        self.engine.connect_unblock(
            self.server_addr, self._on_server_connected, __on_connect_error, cb_args=(client, ))
...
#            print "user auth", user, passwd
        ...
        self.engine.read_unblock(
            conn, 2, __cb0)  # on error automatic close connection
