...
#                            res = True
                if res:
                    if conn.error is None:
                        self._poll.replace_read(
                            conn.fd, self._read_ahead, (conn, ))
                    self._conn_callback(
                        conn, conn.readable_cb, (conn, ) + conn.readable_cb_args, count=1)
                    return
            try:
                ...
                self.logger.error(
                    "peer %s: watch conn error %s" % (conn.peer, str(e)))
        else:
            self._lock()
    ...
    def listen(self, sock, readable_cb, readable_cb_args=(),
        ...
        try:
            ...
            self.log_error(
                "cannot set FD_CLOEXEC on listening socket fd, %s" % (str(e)))
    ...
    def _do_unblock_read(self, conn, ok_cb, direct=False):
        ...
        if conn.error is None:
            self._poll.replace_read(conn.fd, self._read_ahead, (conn, ))
            self._conn_callback(
                conn, ok_cb, (conn, ) + conn.read_cb_args, stack=conn.read_tb)
        else:
            self._conn_callback(conn, conn.read_err_cb, (
                conn, ) + conn.read_cb_args, stack=conn.read_tb)
...
#                    self._unlock ()
        ...
        self._conn_callback(conn, conn.error is None and ok_cb or conn.write_err_cb, (
    ...
    def readline_unblock(self, conn, max_len, ok_cb, err_cb=None, cb_args=()):
        ...
        if self._do_unblock_readline(conn, ok_cb, max_len, direct=True):
            self._conn_callback(
                conn, conn.error is None and ok_cb or conn.read_err_cb, (conn, ) + conn.read_cb_args, count=2)
    ...
    def write_unblock(self, conn, buf, ok_cb, err_cb=None, cb_args=()):
        ...
        if self._do_unblock_write(conn, buf, ok_cb, direct=True):
            self._conn_callback(
                conn, conn.error is None and ok_cb or conn.write_err_cb, (conn, ) + conn.write_cb_args, count=2)
    ...
    def _conn_callback(self, conn, cb, args, stack=None, count=1):
        if conn.error is not None:
            self.close_conn(
                conn)  # NOTICE: we will close the conn before err_cb
        if not callable(cb):
            return
